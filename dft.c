...
void fourier(data_t *a, complex_t *b, int N, int nyquistLimit, FILE *r)
{
  int n, k;
  complex_t point, coefficient, product;
  double magnitude, frequency_resolution = 0;
  
  for(k = 0; k < nyquistLimit; k++) {
    for(n = 0; n < N; n++) {
      point.real = a[n].y;
      point.imm = 0;
    ...
    fprintf(r, "%f %f\n", frequency_resolution, magnitude * 2.0 / N);
    frequency_resolution += 2.0 * nyquistLimit / N;
  }
}
...
int main(int argc, char** argv)
{
  ...
  data_t *read;
  complex_t *bins;
  int k, N_samples = 0, samplingFreq, nyquistLimit;
  char ch;
  ...
  samplingFreq = (int)(1.0 / read[1].t);
  nyquistLimit = samplingFreq / 2;
  
  bins = malloc(sizeof(complex_t) * nyquistLimit);
  for(k = 0; k < nyquistLimit; k++) {
    bins[k].real = 0;
    bins[k].imm = 0;
  }
  
  fourier(read, bins, N_samples, nyquistLimit, results);
